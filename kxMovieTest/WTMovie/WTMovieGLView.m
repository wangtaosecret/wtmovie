...
NSString *const wtvertexShaderString = SHADER_STRING(
...
NSString *const wtrgbFragmentShaderString = SHADER_STRING
(
...
NSString *const wtyuvFragmentShaderString = SHADER_STRING
(
 varying highp vec2 v_texcoord;
...
//解析器协议
@protocol WTMovieGLRenderer
- (BOOL) isValid;
- (NSString *) fragmentShader;
- (void) resolveUniforms: (GLuint) program;
- (void) setFrame: (WTVideoFrame *) frame;
- (BOOL) prepareRender;
@end
//rgb格式解析器
@interface WTMovieGLRenderer_RGB : NSObject<WTMovieGLRenderer>{
	GLint _uniformSampler;	//取样
	GLuint _texture;	//纹路
}
@end
@implementation WTMovieGLRenderer_RGB
//判断解析器是否有效
- (BOOL)isValid{
	return (_texture !=0);
}
//帧着色器
- (NSString *)fragmentShader{
	return wtrgbFragmentShaderString;
}
- (void)resolveUniforms:(GLuint)program{
...
- (void)setFrame:(WTVideoFrame *)frame{
	...
	//需要断言，这里的条件是rgb的总容量=width * height * 3 http://blog.csdn.net/leixiaohua1020/article/details/50534150
//	如果视频帧的宽和高分别为w和h，那么一帧YUV444P像素数据一共占用w*h*3 Byte的数据。其中前w*h Byte存储Y，接着的w*h Byte存储U，最后w*h Byte存储V
	...
	//设置像素储存模式，GL_UNPACK_ALIGNMENT 从内存中读取也就是读取数据保存像素 GL_PACK_ALIGNMENT读取到内存
...
@interface WTMovieGLRenderer_YUV : NSObject<WTMovieGLRenderer>{
	
	GLint _uniformSamplers[3];
	GLuint _textures[3];
}
@end
@implementation WTMovieGLRenderer_YUV
- (BOOL)isValid{
	return (_textures[0] !=0);
}
- (NSString *)fragmentShader{
	return wtyuvFragmentShaderString;
}
- (void)resolveUniforms:(GLuint)program{
...
- (void)setFrame:(WTVideoFrame *)frame{
	...
	WTVideoFrameYUV *yuvFrame = (WTVideoFrameYUV *)frame;
...
@implementation WTMovieGLView{
	...
	id<WTMovieGLRenderer> _renderer;
}
+ (Class) layerClass
{
			 ...
			 decoder: (WTMovieDecoder *) decoder
{
	...
	if (self) {
		...
		if ([decoder setupVideoFrameFormat:WTVideoFrameFormatYUV]) {
			
			_renderer = [[WTMovieGLRenderer_YUV alloc] init];
			LoggerVideo(1, @"OK use YUV GL renderer");
			
		} else {
			
			_renderer = [[WTMovieGLRenderer_RGB alloc] init];
...
- (BOOL)loadShaders
{
	...
	vertShader = compileShader(GL_VERTEX_SHADER, wtvertexShaderString);
	if (!vertShader)
		goto exit;

...
//定义error的域
NSString * wtmovieErrorDomain = @"com.dewmobile.wtmovie";
static void FFLog(void* context, int level, const char* format, va_list args);
//创建error
static NSError * wtmovieError (NSInteger code, id info)
{
...
@interface WTMovieFrame()
@property (nonatomic, assign) CGFloat position;
@property (nonatomic, assign) CGFloat duration;
@end
@implementation WTMovieFrame
@end
...
- (WTMovieFrameType)type{
	return WTMovieFrameTypeAudio;
}
@end
...
- (WTMovieFrameType)type{
	return WTMovieFrameTypeVideo;
}
@end

@interface WTVideoFrameRGB()
@property (nonatomic, assign) NSUInteger linesize;
@property (nonatomic, strong) NSData *rgb;

@end
@implementation WTVideoFrameRGB
- (WTVideoFrameFormat)format{
	return WTVideoFrameFormatRGB;
}
- (UIImage *) asImage
{
	UIImage *image = nil;
...
@interface WTVideoFrameYUV()
@property (nonatomic, strong) NSData *luma;
@property (nonatomic, strong) NSData *chromaB;
@property (nonatomic, strong) NSData *chromaR;
@end
@implementation WTVideoFrameYUV
- (WTVideoFrameFormat)format{
	return WTVideoFrameFormatYUV;
}
@end


@interface WTArtworkFrame()
@property (nonatomic, strong) NSData *picture;
@end
@implementation WTArtworkFrame
- (WTMovieFrameType)type{
	return WTMovieFrameTypeArtwork;
}
- (UIImage *) asImage
{
	UIImage *image = nil;
...
@interface WTSubtitleFrame()
@property (nonatomic, strong) NSString *text;
@end
@implementation WTSubtitleFrame
- (WTMovieFrameType)type{
	return WTMovieFrameTypeSubtitle;
}
@end


@interface WTMovieDecoder(){
	AVFormatContext		*_formatCtx; 		//视频上下文
	AVCodecContext		*_videoCodecCtx;	//视频编解码上下文
	AVCodecContext		*_audioCodecCtx;	//音频编解码上下文
	AVCodecContext		*_subtitleCodecCtx; //字幕编解码上下文
	//AVFrame结构体一般用于存储原始数据，即非压缩数据，例如对于视频来说是YUV，RGB，对音频来说是PCM，此外还包含一些相关的信息。
//	比如说，解码的时候存储了宏块类型表，QP 表，运动矢量表等数据。编码的时候 也存储了相关的数据。因此在使用 FFMPEG 进行码流分析的时候，AVFrame 是一个很重要的结构体。
	AVFrame				*_videoFrame;	//视频的原始数据
	AVFrame				*_audioFrame;	//音频的原始数据
	NSInteger			_videoStream;	//音频流
	NSInteger			_audioStream;	//视频流
	NSInteger			_subtitleStream;	//字幕流
	AVPicture			_picture;	//图片结构体
	BOOL				_pictureValid; //是否是有效的图片
	...
	NSDictionary        *_info;
	WTVideoFrameFormat  _videoFrameFormat;
	NSUInteger          _artworkStream;
	NSInteger           _subtitleASSEvents;
}
@end
@implementation WTMovieDecoder
@dynamic duration;
@dynamic position;
@dynamic frameWidth;
@dynamic frameHeight;
...
//		av_seek_frame(_formatCtx, _videoStream, ts, AVSEEK_FLAG_FRAME);
...
//		av_seek_frame(_formatCtx, _audioStream, ts, AVSEEK_FLAG_FRAME);
...
- (void) setSelectedAudioStream:(NSInteger)selectedAudioStream
{
	...
	WTMovieError errCode = [self openAudioStream: audioStream];
	if (WTMovieErrorNone != errCode) {
...
- (void) setSelectedSubtitleStream:(NSInteger)selected
{
	...
	if (selected == -1) {
		...
		WTMovieError errCode = [self openSubtitleStream:subtitleStream];
		if (WTMovieErrorNone != errCode) {
...
{
	WTMovieDecoder *mp = [[WTMovieDecoder alloc] init];
	if (mp) {
			...
			error: (NSError **) perror
{
	...
	_path = path;
	//尝试打开目标文件，接受错误
	WTMovieError errCode = [self openInput: path];
	
	if (errCode == WTMovieErrorNone) {
		
		WTMovieError videoErr = [self openVideoStream];
		WTMovieError audioErr = [self openAudioStream];
		
		_subtitleStream = -1;
		
		if (videoErr != WTMovieErrorNone &&
			audioErr != WTMovieErrorNone) {
	...
	if (errCode != WTMovieErrorNone) {
		...
		if (perror)
			*perror = wtmovieError(errCode, errMsg);
		return NO;
	}
...
/**
 ...
 @param path 传入的文件路径，本地或者网络
 @return 打开文件的错误
 */
- (WTMovieError) openInput: (NSString *) path
{
	...
	if (_interruptCallback) {
		...
		//创建失败返回错误
		if (!formatCtx)
			return WTMovieErrorOpenFile;
		//创建成功继续执行，formatCtx的打断回调进行赋值
		AVIOInterruptCB cb = {interrupt_callback, (__bridge void *)(self)};
		formatCtx->interrupt_callback = cb;
	}
	//打开文件失败，释放formatCtx的内存，返回错误
	int errorCode = avformat_open_input(&formatCtx, [path cStringUsingEncoding: NSUTF8StringEncoding], NULL, NULL);
	char buf[] = "";
	...
	//检查视频流不存在，这里因为已经创建了input，所以释放formatCtx需要使用avformat_close_input来释放内存，返回错误
	if (avformat_find_stream_info(formatCtx, NULL) < 0) {
		
		avformat_close_input(&formatCtx);
		return WTMovieErrorStreamInfoNotFound;
	}
	//打印出formatCtx的信息
	av_dump_format(formatCtx, 0, [path.lastPathComponent cStringUsingEncoding: NSUTF8StringEncoding], false);
	//对self的formatCtx赋值
	_formatCtx = formatCtx;
	return WTMovieErrorNone;
}

/**
 打开视频流

 @return 打开错误，没有错误返回 WTMovieErrorNone
 */
- (WTMovieError) openVideoStream
{
	WTMovieError errCode = WTMovieErrorStreamNotFound;
	_videoStream = -1;
	_artworkStream = -1;
...
- (WTMovieError) openVideoStream: (NSInteger) videoStream
{
	...
	if (!codec)
		return WTMovieErrorCodecNotFound;
	...
	if (avcodec_open2(codecCtx, codec, NULL) < 0)
		return WTMovieErrorOpenCodec;
	
	_videoFrame = av_frame_alloc();
	
	if (!_videoFrame) {
		avcodec_close(codecCtx);
		return WTMovieErrorAllocateFrame;
	}
...
- (WTMovieError) openAudioStream: (NSInteger) audioStream
{
...
- (WTMovieError) openSubtitleStream: (NSInteger) subtitleStream
{
	...
	if (codecDesc && (codecDesc->props & AV_CODEC_PROP_BITMAP_SUB)) {
		...
		return WTMovieErrorUnsupported;
	}
	...
	if (codecCtx->subtitle_header_size) {
		...
		if (s.length) {
			...
			NSArray *fields = [WTMovieSubtitleASSParser parseEvents:s];
...
- (WTVideoFrame *) handleVideoFrame
{
	...
	if (_videoFrameFormat == WTVideoFrameFormatYUV) {
		
		WTVideoFrameYUV * yuvFrame = [[WTVideoFrameYUV alloc] init];
		...
		WTVideoFrameRGB *rgbFrame = [[WTVideoFrameRGB alloc] init];
...
- (WTSubtitleFrame *) handleSubtitle: (AVSubtitle *)pSubtitle
{
	NSMutableString *ms = [NSMutableString string];
		...
		if (rect) {
			...
			if (rect->text) { // rect->type == SUBTITLE_TEXT
				...
				if (s.length) {
					...
					NSArray *fields = [WTMovieSubtitleASSParser parseDialogue:s numFields:_subtitleASSEvents];
					if (fields.count && [fields.lastObject length]) {
						
						s = [WTMovieSubtitleASSParser removeCommandsFromEventText: fields.lastObject];
						if (s.length) [ms appendString:s];
					}
				}
			}
	...
	WTSubtitleFrame *frame = [[WTSubtitleFrame alloc] init];
	frame.text = [ms copy];
...
{
	if (format == WTVideoFrameFormatYUV &&
		_videoCodecCtx &&
		(_videoCodecCtx->pix_fmt == AV_PIX_FMT_YUV420P || _videoCodecCtx->pix_fmt == AV_PIX_FMT_YUVJ420P)) {
		
		_videoFrameFormat = WTVideoFrameFormatYUV;
		return YES;
	}
	
	_videoFrameFormat = WTVideoFrameFormatRGB;
		...
		if (packet.stream_index ==_videoStream) {
			...
			while (pktSize > 0) {
				...
				if (gotframe) {
					...
					WTVideoFrame *frame = [self handleVideoFrame];
					if (frame) {
			...
			if (packet.size) {
				...
				WTArtworkFrame *frame = [[WTArtworkFrame alloc] init];
			...
			while (pktSize > 0) {
				...
				if (gotsubtitle) {
					...
					WTSubtitleFrame *frame = [self handleSubtitle: &subtitle];
					if (frame) {
						[result addObject:frame];
					}
...
static int interrupt_callback(void *ctx)
{
	if (!ctx)
		return 0;
	__unsafe_unretained WTMovieDecoder *p = (__bridge WTMovieDecoder *)ctx;
...
@implementation WTMovieSubtitleASSParser
